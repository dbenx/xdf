...
<table class="table table-hover">
    ...
    if ($meta_setting) foreach ((array)$meta_setting as $mskey => $mts) {
        ...
        <tr id="cid_meta_<?php echo $cid; ?>_<?php echo $mts['key']; ?>">
            <td><input name="metadata[<?php echo $md_index; ?>][name]" type="text" value="<?php echo $msname; ?>"
                       class="span3"/></td>
            <td><input name="metadata[<?php echo $md_index; ?>][key]" type="text" value="<?php echo $mts['key']; ?>"
                       class="span3" readonly="true"/></td>
            <td><input name="metadata[<?php echo $md_index; ?>][value]" type="text" value="<?php echo $msvalue; ?>"
                       class="span6"/>
                <button class="btn btn-small btn-danger del_meta" type="button"><i class="fa fa-trash-o"></i> 删除
                </button>
            </td>
        </tr>
        <?php ++$md_index;
    ...
    <?php if (apps_meta::$data) foreach ((array)apps_meta::$data as $mdkey => $mdata) { ?>
        <tr>
            <td><input name="metadata[<?php echo $md_index; ?>][name]" type="text" value="<?php echo $mdata['name']; ?>"
                       class="span3"/></td>
            <td><input name="metadata[<?php echo $md_index; ?>][key]" type="text" value="<?php echo $mdata['key']; ?>"
                       class="span3"/></td>
            <td><input name="metadata[<?php echo $md_index; ?>][value]" type="text"
                       value="<?php echo $mdata['value']; ?>" class="span6"/>
                <button class="btn btn-small btn-danger del_meta" type="button"><i class="fa fa-trash-o"></i> 删除
                </button>
            </td>
        </tr>
