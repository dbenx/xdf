    ...
    <div class="widget-box">
        <div class="widget-title">
            ...
            <ul class="nav nav-tabs" id="apps-add-tab">
                <li class="active"><a href="#apps-add-base" data-toggle="tab"><i class="fa fa-info-circle"></i> 基本信息</a>
                </li>
                ...
                <?php if ($rs['table']) foreach ($rs['table'] as $key => $tval) { ?>
                    <li><a href="#apps-add-<?php echo $key; ?>-field" data-toggle="tab"><i
                                    class="fa fa-database"></i> <?php echo $tval['label'] ? $tval['label'] : $tval['name']; ?>
                            表字段</a></li>
                <?php } ?>
                <?php if ($rs['config']['iFormer']) { ?>
                    <?php if (!$rs['table']) { ?>
                        <li id="tab-field"><a href="#apps-add-field" data-toggle="tab"><i class="fa fa-cog"></i>
                                基础字段</a></li>
                    <?php } ?>
                    <li id="tab-custom"><a href="#apps-add-custom" data-toggle="tab"><i class="fa fa-cog"></i> 字段编辑器</a>
                    </li>
                <?php } ?>
            </ul>
        </div>
        <div class="widget-content nopadding">
            <form action="<?php echo APP_FURI; ?>&do=save" method="post" class="form-inline" id="iCMS-apps"
                  target="iPHP_FRAME">
                <input name="_id" type="hidden" value="<?php echo $this->id; ?>"/>
                <input name="apptype" type="hidden" value="<?php echo $rs['apptype']; ?>"/>
                <input id="config_iFormer" name="config[iFormer]" type="hidden"
                       value="<?php echo $rs['config']['iFormer']; ?>"/>
                <div id="apps-add" class="tab-content">
                    <div id="apps-add-base" class="tab-pane active">
                        <div class="input-prepend">
                            <span class="add-on">应用名称</span>
                            <input type="text" name="_name" class="span3" id="_name"
                        ...
                        <div class="input-prepend">
                            ...
                            <input type="text" name="_title" class="span3" id="_title"
                        ...
                        <div class="input-prepend">
                            ...
                            <textarea name="config[info]" id="config_info" class="span6"
                                      style="height: 150px;"><?php echo $rs['config']['info']; ?></textarea>
                        </div>
                        <div class="clearfloat mb10"></div>
                        <div class="input-prepend">
                            <span class="add-on">模板标签</span>
                            <textarea name="config[template]" id="config_template" class="span6" style="height: 150px;"
                                      readonly><?php echo $rs['config']['template']; ?></textarea>
                        </div>
                        <span class="help-inline">程序自动获取</span>
                        <div class="clearfloat mb10"></div>
                        <div class="input-prepend">
                            <span class="add-on">应用版本</span>
                            <input type="text" name="config[version]" class="span3" id="config_version"
                                   value="<?php echo $rs['config']['version'] ? $rs['config']['version'] : 'v1.0.0'; ?>"/>
                        </div>
                        <span class="help-inline">版本号</span>
                        <div class="clearfloat mb10"></div>
                        <div class="input-prepend">
                            <span class="add-on">应用菜单</span>
                            <select name="config[menu]" id="config_menu" class="chosen-select span3"
                                    data-placeholder="请选择应用类型...">
                                ...
                                <optgroup label="应用菜单">
                                    ...
                                    foreach (menu::$menu_array as $key => $value) {
                                        ...
                                        ?>
                                        <option value="<?php echo $key ?>"><?php echo $value['caption'] ?>
                                            菜单[<?php echo $key ?>]
                                        </option>
                                    <?php } ?>
                                </optgroup>
                        ...
                        <script>$(function () {
                                iCMS.select('config_menu', "<?php echo $rs['config']['menu'] ? $rs['config']['menu'] : '0'; ?>");
                            })</script>
                        ...
                        <div class="input-prepend">
                            ...
                            <div class="switch" data-on-label="启用" data-off-label="禁用">
                                <input type="checkbox" data-type="switch" name="usercp"
                                       id="usercp" <?php echo $rs['config']['usercp'] ? 'checked' : ''; ?>/>
                            </div>
                        </div>
                        ...
                        <div class="input-prepend">
                            ...
                            <div class="switch" data-on-label="启用" data-off-label="禁用">
                                <input type="checkbox" data-type="switch" name="status"
                                       id="status" <?php echo $rs['status'] ? 'checked' : ''; ?>/>
                            </div>
                        ...
                        <?php if (false) { ?>
                            <div class="input-prepend">
                                ...
                                <div class="switch" data-on-label="是" data-off-label="否">
                                    <input type="checkbox" data-type="switch" name="create"
                                           id="create" <?php echo $rs['create'] ? 'checked' : ''; ?>/>
                                </div>
                            </div>
              ...
              一般用于二次开发添加应用.
                        ...
                        <table class="table table-bordered bordered" style="width:720px;">
                            ...
                            <?php if ($rs['table']) { ?>
                                <?php foreach ((array)$rs['table'] as $tkey => $tval) { ?>
                                    <tr>
                                        <td><input type="text" name="table[<?php echo $tkey; ?>][0]" class="span2"
                                                   id="table_<?php echo $tkey; ?>_0"
                                                   value="<?php echo $tval['name']; ?>"/></td>
                                        <td><input type="text" name="table[<?php echo $tkey; ?>][1]" class="span2"
                                                   id="table_<?php echo $tkey; ?>_1"
                                                   value="<?php echo $tval['primary']; ?>"/></td>
                                        <td><input type="text" name="table[<?php echo $tkey; ?>][2]" class="span2"
                                                   id="table_<?php echo $tkey; ?>_2"
                                                   value="<?php echo $tval['union']; ?>"/></td>
                                        <td><input type="text" name="table[<?php echo $tkey; ?>][3]" class="span2"
                                                   id="table_<?php echo $tkey; ?>_3"
                                                   value="<?php echo $tval['label']; ?>"/></td>
                                        <td class="<?php echo $type_3_css; ?>">
                                            <button class="btn btn-small btn-danger del_table" type="button"><i
                    ...
                    <div id="apps-add-menu" class="tab-pane">
                        ...
                        <div class="input-prepend">
                            ...
                            <textarea name="menu" id="menu" class="span8"
                                      style="height:450px;font-family: Consolas,monospace;"><?php echo $rs['menu']; ?></textarea>
                        </div>
                        <div class="clearfloat mb10"></div>
                        <div class="input-prepend">
                            <span class="add-on">内容网址</span>
                            <textarea name="config[iurl]" id="config_iurl" class="span8"
                                      style="height:120px;"><?php echo $rs['config']['iurl'] ? cnjson_encode($rs['config']['iurl']) : ''; ?></textarea>
                        </div>
                    </div>
                    <div id="apps-add-router" class="tab-pane">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody id="routerList">
                            <?php $ridx = 0;
                            if (is_array($rs['router'])) foreach ($rs['router'] as $key => $router) { ?>
                                <tr>
                                    <td><input type="text" name="router[<?php echo $ridx; ?>][0]"
                                               value="<?php echo $key; ?>"/></td>
                                    <td><input type="text" name="router[<?php echo $ridx; ?>][1]"
                                               value="<?php echo $router[0]; ?>"/></td>
                                    <td><input type="text" name="router[<?php echo $ridx; ?>][2]"
                                               value="<?php echo $router[1]; ?>"/></td>
                                    <td>
                                        <button class="btn btn-small btn-danger del_router" type="button"><i
                                                    class="fa fa-trash-o"></i> 删除
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div class="form-actions">
                        ...
                        <?php if ($rs['apptype']) { ?>
                            <a href="<?php echo APP_FURI; ?>&do=uninstall&id=<?php echo $rs['id']; ?>"
                               target="iPHP_FRAME" class="del btn btn-small btn-danger" title='永久删除'
                               onclick="return confirm('卸载应用会清除应用所有数据！\n卸载应用会清除应用所有数据！\n卸载应用会清除应用所有数据！\n确定要卸载?\n确定要卸载?\n确定要卸载?');"/>
                            <i class="fa fa-trash-o"></i> 卸载</a>
                        <?php } ?>
                    </div>
            </form>
        </div>
