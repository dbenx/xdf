...
<style>
    ...
    .store-card .desc, .store-card .name {
        margin-left: 148px;
        margin-right: 120px
    }
    ...
    .store-card-bottom .star-rating {
        display: inline;
        color: #ffb900
    }

    .store-card-update-failed .update-now {
        font-weight: 600
    }

    .store-card-update-failed .notice-error {
        margin: 0;
        padding-left: 16px;
        box-shadow: 0 -1px 0 #ddd
    }

    .store-card-update-failed .store-card-bottom {
        display: none
    }

    .store-card .column-rating {
        line-height: 23px
    }

    .store-card .column-rating, .store-card .column-updated {
        margin-bottom: 4px
    }

    .store-card .column-downloaded, .store-card .column-rating {
        float: left;
        clear: left;
        max-width: 180px
    }

    .store-card .column-compatibility, .store-card .column-updated {
        text-align: right;
        float: right;
        clear: right;
        width: 65%;
    ...
    @media (max-width: 1000px) {
        ...
        .store-action-buttons li .clearfix {
            display: none;
        }

        .store-card .column-rating, .store-card .column-downloaded, .store-card .column-compatibility, .store-card .column-updated {
            width: 100%;
            text-align: left;
            float: none;
        }
    ...
    <div class="widget-box" id="<?php echo APP_BOXID; ?>">
        <div class="widget-title">
            ...
            <ul class="nav nav-tabs" id="config-tab">
                <li <?php if (!isset($_GET['premium'])) echo 'class="active"'; ?>><a
                            href="<?php echo admincp::uri(null, $uriArray); ?>">全部</a></li>
                <li <?php if ($_GET['premium'] == '0') echo 'class="active"'; ?>><a
                            href="<?php echo admincp::uri("premium=0", $uriArray); ?>">免费<?php echo $title; ?></a></li>
                <li <?php if ($_GET['premium'] == '1') echo 'class="active"'; ?>><a
                            href="<?php echo admincp::uri("premium=1", $uriArray); ?>">付费<?php echo $title; ?></a></li>
            </ul>
        </div>
                    ...
                    <div id="store-<?php echo $value['app']; ?>" class="store-card">
                        <div class="store-card-top">
                            <div class="name column-name">
                                <h3>
                                    <a href="<?php echo $value['url']; ?>?modal"
                                       ...
                                       data-meta='{"width":"700px","height":"640px"}'>
                                ...
                                <ul class="store-action-buttons">
                                    <li>
                                        <?php if ($appconf) { ?>
                                            ...
                                            <?php } else { ?>
                                                <a disabled="disabled" href="javascript:;" class="btn btn-default"><i
                                                            class="fa fa-repeat"></i> 暂无更新</a>
                                            <?php } ?>
                                    ...
                                    <?php if ($value['premium']) { ?>
                                        <li class="premium">
                                            <?php if ($value['coupon']) { ?>
                                                <span class="label label-inverse"><del>原价：<i
                                                                class="fa fa-rmb"></i> <?php echo $value['price']; ?></del></span>
                                                <span class="label label-warning">优惠价：<i
                                                            class="fa fa-rmb"></i> <?php echo $value['coupon']; ?></span>
                                            <?php } else { ?>
                                                <span class="label label-success">价格：<i
                                                            class="fa fa-rmb"></i> <?php echo $value['price']; ?></span>
                                            <?php } ?>
                                        </li>
                                    <?php } ?>
                                    <li>
                            ...
                            <div class="desc column-description">
                                ...
                                <p class="authors">
                                    <cite>由<a href="<?php echo $value['website'] ?: 'javascript:;'; ?>"
                                              target="_blank"><?php echo $value['author']; ?></a>提供</cite>
                            ...
                            <?php if ($value['iCMS_GIT_TIME']) { ?>
                                <div class="column-compatibility">
                                    <?php if (GIT_TIME < $value['iCMS_GIT_TIME']) { ?>
                                        <span class="compatibility-untested"><i
                                                    class="fa fa-times"></i><strong>要求</strong>[git:<?php echo get_date($value['iCMS_GIT_TIME'], 'Y-m-d H:i'); ?>]及之后的开发版本</span>
                                    <?php } ?>
                                </div>
                            <?php } ?>
                        </div>
