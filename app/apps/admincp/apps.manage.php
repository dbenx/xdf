    ...
    <div class="widget-box">
        <div class="widget-title"><span class="icon"> <i class="fa fa-search"></i> </span>
            ...
            <div class="pull-right">
                <a style="margin: 10px;" class="btn btn-success btn-mini" href="<?php echo APP_FURI; ?>&do=cache"
                   target="iPHP_FRAME"><i class="fa fa-refresh"></i> 更新缓存</a>
            </div>
        </div>
      ...
      <input type="checkbox" class="checkAll" data-target="#<?php echo APP_BOXID; ?>"/>
            ...
            <ul class="nav nav-tabs" id="apps-tab">
                <?php foreach (apps::$type_array as $key => $value) { ?>
                    <li class="apps-type-<?php echo $key; ?>"><a href="#apps-type-<?php echo $key; ?>"
                                                                 data-toggle="tab"><i
                                    class="fa fa-cubes"></i> <?php echo $value; ?></a></li>
                <?php } ?>
            </ul>
        </div>
            ...
            <form action="<?php echo APP_FURI; ?>&do=batch" method="post" class="form-inline"
                ...
                <div class="tab-content">
                    <?php foreach (apps::$type_array as $type_key => $type_value) { ?>
                        <div id="apps-type-<?php echo $type_key; ?>"
                             class="tab-pane apps-type-<?php echo $type_key; ?>">
                                        ...
                                        <td>
                                            <?php if ($data['type']) { ?>
                                                <?php if ($data['apptype'] == "2") { ?>
                                                    <a href="<?php echo __ADMINCP__; ?>=<?php echo $data['app']; ?>&do=manage&appid=<?php echo $data['id']; ?>"
                                                       class="btn btn-small" target="_blank"><i
                                                                class="fa fa-dashboard"></i> 内容管理</a>
                                                    <a href="<?php echo __ADMINCP__; ?>=<?php echo $data['app']; ?>&do=add&appid=<?php echo $data['id']; ?>"
                                                       class="btn btn-small" target="_blank"><i class="fa fa-edit"></i>
                                                        添加内容</a>
                                                    <div class="clearfix mt5"></div>
                                                <?php } ?>
                                                <a href="<?php echo APP_URI; ?>&do=add&id=<?php echo $data['id']; ?>"
                                                   class="btn btn-small"><i class="fa fa-edit"></i> 编辑</a>
                                                <a href="<?php echo APP_URI; ?>&do=pack&id=<?php echo $data['id']; ?>"
                                                   class="btn btn-small"><i class="fa fa-download"></i> 打包</a>
                                                <?php if ($data['apptype']) { ?>
                                                    <?php if ($data['status']) { ?>
                                                        <a href="<?php echo APP_URI; ?>&do=update&_args=status:0&id=<?php echo $data['id']; ?>"
                                                           target="iPHP_FRAME" class="btn btn-small btn-warning"
                                                           onclick="return confirm('关闭应用不会删除数据，但应用将不可用\n确定要关闭应用?');"><i
                                                                    class="fa fa-close"></i> 关闭</a>
                                                    <?php } else { ?>
                                                        <a href="<?php echo APP_URI; ?>&do=update&_args=status:1&id=<?php echo $data['id']; ?>"
                                                           target="iPHP_FRAME" class="btn btn-small btn-success"><i
                                                                    class="fa fa-check"></i> 启用</a>
                                                    <?php } ?>
                                                    <a href="<?php echo APP_FURI; ?>&do=uninstall&id=<?php echo $data['id']; ?>"
                                                       target="iPHP_FRAME" class="del btn btn-small btn-danger"
                                                       title='永久删除'
